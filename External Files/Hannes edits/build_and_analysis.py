...
ring_tw=ring.twiss(method='6d', radiation_integrals=True, eneloss_and_damping=True,
                   spin=True, polarization=True, at_s=np.linspace(0, ring.get_length(), 5000, endpoint=False) )
...
# ring.survey().plot()
# plt.tight_layout()
# plt.savefig(f'config_D/ring_survey.png')

ring_tw.plot(f'delta (zeta+0.00504) x')
plt.tight_layout()
plt.savefig(f'config_D/ring_delta.png')

ring_tw.plot()
plt.tight_layout()
plt.savefig(f'config_D/ring_twiss.png')
...
data = [
    ...
    ["Horizontal Tune (Qx)", f"{ring_tw.qx:10.3f}", ""],
    ["Vertical Tune (Qy)", f"{ring_tw.qy:10.3f}", ""],
    ...
    ["Horiz. Damping Constant", f"{1e3/ring_tw.damping_constants_s[0]:.5f}", "ms"],
    ["Vert. Damping Constant", f"{1e3/ring_tw.damping_constants_s[1]:.5f}", "ms"],
    ["Long. Damping Constant", f"{1e3/ring_tw.damping_constants_s[2]:.5f}", "ms"]
]
...
plt.savefig(f'config_D/table_results_D.png')
# mf.SpuckParsAus( period_sliced.twiss(method='4d'), period,(0., 38.), (0., 12.), (0., 1.), .08,pdr, f"config_D/Standard.png" )
...
pdr.vars['k2XFarc_delta'] = 0
pdr.vars['k2XDarc_delta'] = 0

print('2nd order chrom x', ring_tw.ddqx)
print('2nd order chrom y', ring_tw.ddqy)

# pdr.vars['k2XFarc_delta'] = 13
# pdr.vars['k2XDarc_delta'] = -11
# pdr.vars['k2XFarc_delta'] = 11
# pdr.vars['k2XDarc_delta'] = -15
# pdr.vars['k2XFarc_delta'] = 16
# pdr.vars['k2XDarc_delta'] = -11
pdr.vars['k2XFarc_delta'] = 17.7
pdr.vars['k2XDarc_delta'] = -10
# pdr.vars['k2XFarc_delta'] = 18
# pdr.vars['k2XDarc_delta'] = -2
# pdr.vars['k2XFarc_delta'] = 18
# pdr.vars['k2XDarc_delta'] = -5
# pdr.vars['k2XFarc_delta'] = 3
# pdr.vars['k2XDarc_delta'] = -9

# # pdr.vars['k2XFarc_delta'] = -5   #7
# pdr.vars['k2XDarc_delta'] = -8

# pdr.vars['k2XFarc_delta'] = 17
# pdr.vars['k2XDarc_delta'] = -10

# pdr.vars['k2XFarc_delta'] = 45
# pdr.vars['k2XDarc_delta'] = 5

# pdr.vars['k2XFarc_delta'] = 13+4
# pdr.vars['k2XDarc_delta'] = -10#-10

ring_tw=ring.twiss(method='6d', radiation_integrals=True, eneloss_and_damping=True,
                   spin=True, polarization=True, at_s=np.linspace(0, ring.get_length(), 1000, endpoint=False) )
print('2nd order chrom x', ring_tw.ddqx)
print('2nd order chrom y', ring_tw.ddqy)
# mf.SpuckParsAus( period_sliced.twiss(method='4d'), period,(0., 38.), (0., 12.), (0., 1.), .08,pdr, f"config_D/Standard.png" )
...
plt.savefig(f'config_D/momentum_deviation{constants.WP}.png')
...
# amplitude dependent tune footprint
nemitt_x = 1.3e-3
nemitt_y = 1.2e-3
r_max = 15
...
fp0 = ring.get_footprint(nemitt_x=nemitt_x, nemitt_y=nemitt_y, r_range=r_range, n_r=n_r)
fp0.qx += np.floor(WP[0])
fp0.qy += np.floor(WP[1])
fp0.plot(color='orange', label='up to %d sigma'%r_max)

ax.plot(qx, qy, '.-', label='%1.1e $< \delta <$ %1.1e'%(min(delta),max(delta)), color='g')
# Plot the actual center (delta=0)
ax.plot(nominal_tw.qx,nominal_tw.qy, 'go', label='Nominal ($\delta=0$)', ms=9)
ax.legend()
resonances.plot_resonance(fig)
plt.tight_layout()
plt.savefig(f'config_D/momentum_dev_working_point.png')
...
axs[0].grid()
axs[1].set_xlim(tw.circumference/6,3*tw.circumference/6)
axs[2].set_ylim(0,3)
